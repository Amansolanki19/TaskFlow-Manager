...
public class TaskService {
    ...
    public List<Task> getByTask(String task){
        return taskRepository.findByTaskContaining(task);
    }
    ...
    public Task updateTask(Long id,Task task) {
        ...
        if(task.getTask()!=null)    task1.setCompleted(task.getCompleted());
        if (task.getTask()!=null)   task1.setTask(task.getTask());
        return taskRepository.save(task);
    }
